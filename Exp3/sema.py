...
def P81():
	...
	e = f.children[2]
	code_output('IF ' + last_token + ' GOTO ' + str(CODE_SIZE + 2))
	code_output(None)
...
def print_symbol_table():
	global analysis
	for each in SYMBOL_TABLE:
		analysis.insert(END, str(each) + '\n')

def print_code_result():
	global analysis
	for each in CODE_RESULT:
		analysis.insert(END,str(CODE_RESULT.index(each)) + ': ' + str(each) + '\n')
...
analysis = ScrolledText(root, width=50, height=30, font=10)
...
def semantic_analysis():
	...
	analysis.insert(END,"SYMBOL TABLE\n")
	analysis.insert(END,"------------\n")
	print_symbol_table()

	analysis.insert(END,"\n")
	analysis.insert(END,"CODE\n")
	analysis.insert(END,"------------\n")
	print_code_result()
...
def interface():
	...
	Analysis = Button(root, text = 'Semantic  Analysis', command = semantic_analysis, font=15)
